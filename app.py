...
masking -> [REDACTED], token-aware Head+Tail truncation). Together with
the Accountability page it implements Research Objective 2 (SDG 16):
real-time classification plus a composite high-risk institution score,
complaint-trend charts, and a model-in-the-loop check that
...
NAV_PAGES = ["🏠 Home", "Live classifier", "Overview", "Dataset", "Training",
             "Evaluation", "Accountability", "Model card"]

page = st.sidebar.radio("Navigate", NAV_PAGES, key="nav")


def _goto(target: str):
    """Callback used by the Home-page cards to jump to a section."""
...
if page == "🏠 Home":
    ...
    st.info(
        "**Research Objective 2:** Deploy the model to build a real-time "
        "classification & institutional accountability dashboard, supporting "
        "**SDG 16** by visualizing complaint trends and flagging high-risk "
    ...
    cards_top = [
        ...
        ("🏛️", "Accountability (SDG 16)", "Accountability",
    ...
    for col, (icon, title, target, desc) in zip(r1, cards_top):
        with col:
            st.markdown(
            ...
            st.button(f"Open {title} →", key=f"go_{target}",
                      on_click=_goto, args=(target,), width="stretch")
    st.markdown("&nbsp;", unsafe_allow_html=True)
    r2 = st.columns(3)
    cards_bot = [
        ("📊", "Dataset", "Dataset",
    ...
    for col, (icon, title, target, desc) in zip(r2, cards_bot):
        with col:
            st.markdown(
                f"<div class='feature-card'><div class='fc-icon'>{icon}</div>"
                f"<h4>{title}</h4><p>{desc}</p></div>",
...
# --------------------------------------------------------------------------- #
    ...
    st.markdown(
        ...
        "|---|---|---|\n"
        "| **RO1** | Preprocess CFPB data and fine-tune DistilBERT for "
        "accurate complaint classification | **Dataset** page (cleaning, "
        "stratified 70/15/15 split, class weights, tokenized rows) and the "
        "**Training** / **Evaluation** pages (loss & macro-F1 curves, "
        "per-class F1, confusion matrix) |\n"
        "| **RO2** | Deploy the model to build a real-time classification & "
        "institutional accountability dashboard, supporting SDG 16 by "
        "visualizing complaint trends and flagging high-risk financial "
        "institutions | **Live classifier** (real-time prediction with the "
        "training pipeline applied at inference) and the **Accountability** "
        "page (composite risk score, complaint trends, model-in-the-loop "
        "check) |"
    )
...
# --------------------------------------------------------------------------- #
    ...
    st.info(
        ...
        "classification & institutional accountability dashboard, supporting "
        "SDG 16 by visualizing complaint trends and flagging high-risk "
        "financial institutions.* This page delivers the accountability half: "
        "the composite **high-risk score**, the **complaint trend** charts "
        "below, and the **model-in-the-loop** check that runs the fine-tuned "
        "DistilBERT over live narratives.",
        icon="🎯",
    )
