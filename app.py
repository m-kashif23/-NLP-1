...
You can either (a) place the project files next to app.py, or (b) just upload
them from the sidebar ("Upload data & model files"). Either works; uploads take
priority. Recognised files:
    ...
    model.safetensors  (or pytorch_model.bin)   -> enables the live classifier
    a raw complaints CSV                         -> raw narrative examples

NOTE: model files can be large. If a file exceeds 200 MB, raise Streamlit's
limit by shipping the included .streamlit/config.toml, or run:
...
UPLOAD_DIR = Path(tempfile.gettempdir()) / "complaint_dashboard_assets"
...
def resolve(name: str):
    """Return a usable path for `name`: uploaded copy first, else local, else None."""
    up = UPLOAD_DIR / name
    if up.exists():
        return up
    local = BASE_DIR / name
...
def classify_upload(filename: str):
    """Map an uploaded filename to a canonical asset name (best-effort)."""
    n = filename.lower()
    if n.endswith(".safetensors"):
        return "model.safetensors"
    if n.endswith(".bin") and "pytorch" in n:
        return "pytorch_model.bin"
    if "tokenizer_config" in n:
        return "tokenizer_config.json"
    if n.endswith(".json") and "tokenizer" in n and "config" not in n:
        return "tokenizer.json"
    if n.endswith(".json") and ("pipeline_metadata" in n or "metadata" in n):
        return "pipeline_metadata.json"
    if n == "config.json" or (
        n.endswith(".json") and "config" in n
        and "tokenizer" not in n and "metadata" not in n and "pipeline" not in n
    ):
        return "config.json"
    if n.endswith(".png") and "confusion" in n:
        return "confusion_matrix.png"
    if n.endswith(".png") and ("training" in n or "curve" in n or "loss" in n):
        return "training_curves.png"
    if n.endswith(".csv") and "train" in n:
        return "processed_train.csv"
    if n.endswith(".csv") and "val" in n:
        return "processed_val.csv"
    if n.endswith(".csv") and "test" in n:
        return "processed_test.csv"
    if n.endswith(".csv") and "complaint" in n:
        return "raw_complaints.csv"
    return None


def save_upload(uploaded, canonical: str):
    """Write an uploaded file to UPLOAD_DIR/canonical, skipping if unchanged."""
    target = UPLOAD_DIR / canonical
    data = uploaded.getbuffer()
    if target.exists() and target.stat().st_size == data.nbytes:
        return  # already saved this exact file
...
# --------------------------------------------------------------------------- #
def find_weights():
    return resolve("model.safetensors") or resolve("pytorch_model.bin")


def ensure_model_dir():
    """
    Gather config + tokenizer + weights into one directory so transformers can
    load from it. Returns (model_dir, missing_list).
    """
    weights = find_weights()
    needed = {
        "config.json": True,
        "tokenizer.json": True,
        "tokenizer_config.json": False,  # optional but copied if present
    }
    missing = []
    for fname, required in needed.items():
        src = resolve(fname)
        if src is None:
            if required:
    ...
    if weights is None:
        ...
        dst = UPLOAD_DIR / weights.name
        if weights.resolve() != dst.resolve():
            shutil.copy(weights, dst)
    return (UPLOAD_DIR if not missing else None), missing


@st.cache_resource(show_spinner="Loading model (first run only)…")
def load_model(model_dir: str, sig):
...
def get_hf_model_id():
    ...
    Set it as a Streamlit secret (Settings -> Secrets):  HF_MODEL_ID = "user/repo"
    or as an environment variable HF_MODEL_ID. Lets the app run on Streamlit
    Cloud without committing the large weights to GitHub.
    """
    try:
        ...
        pass
    return os.environ.get("HF_MODEL_ID")
...
# Detect what we already have so we know whether to auto-open the uploader
KNOWN_ASSETS = [
    "config.json", "tokenizer.json", "tokenizer_config.json",
    "pipeline_metadata.json", "training_curves.png", "confusion_matrix.png",
    "processed_train.csv", "processed_val.csv", "processed_test.csv",
]
detected = {a: resolve(a) is not None for a in KNOWN_ASSETS}
detected["model weights"] = find_weights() is not None
detected["raw complaints csv"] = resolve_any(RAW_CSV_CANDIDATES) is not None
any_detected = any(detected.values())

with st.sidebar.expander("📤 Upload data & model files", expanded=not any_detected):
    st.caption(
        "Drop your files here — datasets, model weights, config/tokenizer and "
        "plots. Files are matched automatically by name."
    )
    ups = st.file_uploader(
        "Add files",
        accept_multiple_files=True,
        type=["csv", "json", "png", "safetensors", "bin"],
        label_visibility="collapsed",
    )
    if ups:
        mapped, unmapped = [], []
        for uf in ups:
            canonical = classify_upload(uf.name)
            if canonical:
                save_upload(uf, canonical)
                mapped.append((uf.name, canonical))
            else:
                unmapped.append(uf.name)
        if mapped:
            st.success(f"Saved {len(mapped)} file(s).")
            st.dataframe(
                pd.DataFrame(mapped, columns=["Uploaded", "Used as"]),
                hide_index=True, use_container_width=True,
            )
        if unmapped:
            st.warning(
                "Couldn't recognise: " + ", ".join(unmapped)
                + ". Rename to include train/val/test, 'config', 'tokenizer', "
                "'confusion' or 'training' so it can be matched."
            )
    # live status
    st.markdown("**Detected assets**")
    status_lines = []
    for key in ["config.json", "tokenizer.json", "model weights",
                "training_curves.png", "confusion_matrix.png",
                "processed_train.csv", "processed_val.csv", "processed_test.csv",
                "raw complaints csv"]:
        ok = detected.get(key, resolve(key) is not None)
        status_lines.append(f"{'✅' if ok else '⬜'} {key}")
    st.markdown("<span class='muted'>" + "<br>".join(status_lines) + "</span>",
                unsafe_allow_html=True)
...
# --------------------------------------------------------------------------- #
    ...
    if sample is not None:
        ...
        st.caption("Upload processed_train.csv to preview a tokenized row.")
    ...
    if raw is not None:
        ...
        st.caption("Upload a raw complaints CSV to show example narratives here.")
...
# --------------------------------------------------------------------------- #
    ...
    with tab1:
        ...
        st.dataframe(
            per_class_df, hide_index=True, use_container_width=True,
            column_config={
...
# --------------------------------------------------------------------------- #
    ...
    model_dir, missing = ensure_model_dir()
    hf_id = get_hf_model_id()

    if model_dir is None and not hf_id:
        st.info(
            "To run live predictions, do one of:\n\n"
            "1. **Upload** the model files from the sidebar (missing: "
            + ", ".join(f"`{m}`" for m in missing) + "), or\n"
            "2. **Load from Hugging Face** — push your model to the Hub and set a "
            "secret `HF_MODEL_ID = \"your-username/your-model\"` "
            "(Streamlit Cloud: Manage app → Settings → Secrets).\n\n"
            "The inference packages (`torch`, `transformers`, `safetensors`) come "
            "from `requirements.txt` automatically.",
            icon="📤",
        )
    else:
        src_note = ("Hugging Face Hub: " + hf_id) if model_dir is None else "local / uploaded files"
        st.caption(f"Model source: {src_note}")
        if st.button("Classify", type="primary", disabled=not text.strip()):
            try:
                ...
                if model_dir is not None:
                    tok, model = load_model(str(model_dir), file_sig(find_weights()))
                else:
...
# --------------------------------------------------------------------------- #
    ...
    if inst_df is None or inst_df.empty:
        st.info(
            "Upload a raw CFPB complaints CSV from the sidebar (any file with "
        ...
        if risk is None or risk.empty:
            ...
            st.dataframe(
                show, hide_index=True, use_container_width=True, height=420,
                column_config={
