...
def attention_attribution(tok, input_ids, attentions):
    """Lightweight token-level attribution (proposal §7.4 / §8):
    average the last encoder layer's heads and read the [CLS] row —
    which tokens the classification summary attends to most."""
    att = attentions[-1].mean(dim=1)[0, 0, :]  # heads averaged, CLS row
    tokens = tok.convert_ids_to_tokens(input_ids[0])
    words, scores = [], []
    special = {tok.cls_token, tok.sep_token, tok.pad_token}
    for t, s in zip(tokens, att.tolist()):
        if t in special:
            continue
        if t.startswith("##") and words:
            words[-1] += t[2:]
            scores[-1] = max(scores[-1], s)
        else:
            words.append(t)
            scores.append(s)
    return words, scores
...
if page == "Overview":
    ...
    st.markdown(
        ...
        "an attention-based view of the most influential tokens."
    )
...
# --------------------------------------------------------------------------- #
    ...
    if st.button("Classify", type="primary", disabled=not text.strip()):
        try:
            ...
            # ---- Explainability: attention-based token attribution -------
            st.markdown("### 🔍 Why this prediction? (attention attribution)")
            words, scores = attention_attribution(tok, input_ids,
                                                  out.attentions)
            if words:
                smax = max(scores) or 1.0
                html = " ".join(
                    f"<span style='background:rgba(59,111,255,{0.05 + 0.55*(s/smax):.2f});"
                    f"border-radius:4px;padding:1px 3px'>{w}</span>"
                    for w, s in zip(words, scores)
                )
                st.markdown(html, unsafe_allow_html=True)
                st.caption(
                    "Darker tokens received more attention from the [CLS] "
                    "summary token in the final encoder layer. This is a "
                    "lightweight transparency aid, not a causal explanation."
                )
...
# --------------------------------------------------------------------------- #
    ...
    # --- data source: uploader takes priority, then local / HF-hosted CSV ---
    def _parse_complaints_csv(src):
        df = pd.read_csv(src, usecols=lambda c: c in INST_COLS,
    ...
    uploaded = st.file_uploader(
        "📤 Upload a complaints dataset (CSV)",
        type=["csv"],
        help=("Upload a CFPB-style CSV with columns: Company, "
              "`Timely response?`, `Consumer disputed?`, Product, Date received. "
              "If you don't upload, the dashboard uses the bundled / Hugging Face copy."),
        key="acct_csv_uploader",
    )

    inst_df = None
    if uploaded is not None:
        try:
            with st.spinner("Parsing your CSV…"):
                inst_df = _parse_complaints_csv(uploaded)
            st.success(f"Loaded {len(inst_df):,} complaints from your upload.")
        except Exception as exc:  # noqa: BLE001
    ...
    if inst_df is None or inst_df.empty:
        st.caption("Upload a complaints CSV above to analyse institution-level risk.")
    else:
        # ---- controls --------------------------------------------------------
